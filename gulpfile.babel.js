...
gulp.task('styletemplates', () => {
    ...
    return gulp.src('src/template.scss')
        ...
        // Inlines images as base64 strings in CSS
        .pipe($.cssInlineImages({
            webRoot: 'src'
        }))
        .pipe($.autoprefixer(AUTOPREFIXER_BROWSERS))
...
gulp.task('metadata', () => {
    console.log('Cranking out the package.json, bower.json, License files');
    return gulp.src([
        'package.json',
        'bower.json',
        'LICENSE'
...
gulp.task('styleguide-github', () => {
    return gulp.src([
        ...
        'dist/LICENSE',
        'dist/material.css',
        'dist/material.css.template',
        'dist/material.js',
        'dist/material.min.css',
        'dist/material.min.css.map',
        'dist/material.min.css.template',
        'dist/material.min.css.template.map',
        'dist/material.min.js',
        'dist/material.min.js.map',
        'dist/material-grid.css',
        'dist/material-grid.min.css',
        'dist/mdl.zip'
    ], {
        base: "dist/"
    })
        .pipe(gulp.dest('dist/vaco-mdl'));
// .pipe(gulp.dest('dist/vaco-mdl')
//     .src('dist/assets/**/*')
//     .pipe(gulp.dest('dist/vaco-mdl/assets'))
//     .src('dist/components/**/*')
//     .pipe(gulp.dest('dist/vaco-mdl/components'))
//     .src('dist/faq/**/*')
//     .pipe(gulp.dest('dist/vaco-mdl/faq'))
//     .src('dist/images/**/*')
//     .pipe(gulp.dest('dist/vaco-mdl/images'))
//     .src('dist/styles/**/*')
//     .pipe(gulp.dest('dist/vaco-mdl/styles')));
});
    ...
    return gulp.src('docs/_pages/*.md')
        ...
        .pipe($.rename(path => {
            if (path.basename !== 'index') {
                ...
                console.log('Path basename is not equal to index? ' + path.dirname);
                path.basename = 'index';
            }
        }))
        .pipe(gulp.dest('dist'));
 ...
 */
gulp.task('assets', () => {
    console.log('STARTING ASSETS!!! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~' + __dirname);
    return gulp.src([
        'docs/_assets/**/*',
...
gulp.task('genCodeFiles', () => {
    console.log('Are we generateing code files?');
    return gulp.src([
